    ...
    $errorsSql = false;
        ...
        if ($status == 0) {
            ...
            $total =  $tempPurchasesSumInvoiceId['totalPurchasePrice'];
                ...
                $sql = "INSERT INTO `invoicePurchases` ( `adminId`, `userId`, `randCode`, `total`, `cash`, `date`, dateRegistered , `time`, `status`) 
                VALUES ( '$adminId', $userId, '$randCode', '$total', '$total', '$date' ,'$TimeCheck' , '$time' , 0 );";
                if ($conn->query($sql) === TRUE) {
                    ...
                    while ($row = $reSQL->fetch_assoc()) {
                        ...
                        $color = $row['color'];
                        $barcode = $row['barcode'];
                        $sql = "INSERT INTO `stock` (randCode , `categoryId`, `modelId`, `adminId`, `userId`, `invoicePurchasesId`, `img`, `salePrice`, `purchasePrice`, `size`, `color`, `status` , `barcode`) 
                        VALUES ('$randCode' , $categoryId, $modelId ,$adminId, $userId, $invoicePurchasesId,
                        'default.jpg', $salePrice, $purchasePrice, '$size', '$color', 0, '$barcode');";
                         $conn->query($sql);
                  }
